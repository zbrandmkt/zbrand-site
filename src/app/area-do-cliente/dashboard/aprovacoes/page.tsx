...
export default async function AprovacoesPage() {
  ...
  const permissions: string[] = (link?.clients as any)?.permissions ?? ["aprovacoes"];
  if (!isAdmin && !permissions.includes("aprovacoes")) {
  ...
  return (
    <div className="px-6 py-8">
      ...
      {allPosts.length === 0 ? (
        <div
          className="bg-white border-2 border-[#1A1A1A] rounded-2xl px-5 py-16 text-center"
        ...
        >
          ...
          <p className="text-xs text-[#1A1A1A]/30 mt-1">
            Quando a ZBRAND enviar posts para aprovação, eles aparecerão aqui.
          </p>
        </div>
      ) : (
