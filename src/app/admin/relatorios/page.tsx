  ...
  return (
    <div className="px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-white/40 mt-1">Edite os dados de tráfego e social de cada cliente</p>
      </div>

      {activeClients.length === 0 ? (
        <div className="bg-white/04 border border-white/08 rounded-2xl px-5 py-16 text-center">
          <p className="text-3xl mb-3">📊</p>
          <p className="text-sm text-white/30 font-medium">Nenhum cliente ativo ainda.</p>
          <p className="text-xs text-white/20 mt-1">Ative clientes na aba Clientes para editar relatórios.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-4">
          {activeClients.map((client) => {
            ...
            return (
              <div
                key={client.id}
                className="bg-white/04 border border-white/08 rounded-2xl p-5 flex flex-wrap items-center justify-between gap-4 hover:border-white/15 transition-colors"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-black text-white">{client.company}</p>
                    <p className="text-xs text-white/40">{client.name}</p>
                    {client.plan && (
                      <span className="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 bg-[#FF6100]/15 text-[#FF6100] rounded-full">
                        {client.plan}
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-6">
                  ...
                  {clientReports.length > 0 && (
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] text-white/30 font-medium mr-1">Meses:</span>
                      {clientReports.slice(0, 6).map((r) => (
                        <span key={r.id} className="text-[10px] font-black px-2 py-0.5 bg-white/08 text-white/60 rounded-full">
